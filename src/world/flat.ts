...
/**
 * A flat test world: bedrock, stone, dirt and grass up to `ground`, with a row
 * of every block on display near the middle.
 */
export function createFlatWorld(sx: number, sy: number, sz: number, ground = 31): World {
  const world = new World(sx, sy, sz, 0);
  const { blocks } = world;
  for (let y = 0; y <= ground; y++) {
    const id = y === 0 ? B.BEDROCK : y < ground - 2 ? B.STONE : y < ground ? B.DIRT : B.GRASS;
    blocks.fill(id, y * sx * sz, (y + 1) * sx * sz);
  }
  ...
  PICKABLE_BLOCKS.forEach((id, i) => {
    const x = x0 + i * 2;
    if (x < 0 || x >= sx) return;
    blocks[world.index(x, ground + 1, cz)] = id;
  });
  world.heightMap.recomputeAll(blocks);
  return world;
}
