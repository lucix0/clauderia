...
export class Ticker {
  ...
  private readonly listener = (x: number, y: number, z: number, oldId: number, newId: number): void =>
    this.onChange(x, y, z, oldId, newId);
  private readonly trees: TreeTarget;

  constructor(private readonly world: World) {
    this.rng = new Rng(world.seed ^ 0x7ac3);
    const { blocks } = world;
    for (let i = 0; i < blocks.length; i++) if (blocks[i] === B.SPONGE) this.sponges.add(i);
  ...
  schedule(x: number, y: number, z: number, delay: number): void {
    ...
    this.scheduleIndex(this.world.index(x, y, z), delay);
  }
  ...
  private onChange(x: number, y: number, z: number, oldId: number, newId: number): void {
    const w = this.world;
    ...
    if (!IS_SOLID[newId]) {
      ...
      for (const [dx, dy, dz] of NEIGHBOURS) {
        ...
        if (wake) this.scheduleIndex(w.index(nx, ny, nz), this.reactionDelay(nx, ny, nz, nid));
      }
    }
    // Grass under a new light-blocking block dies after a while.
    if (BLOCKS_LIGHT[newId] && y > 0 && w.get(x, y - 1, z) === B.GRASS) {
    ...
    if (!BLOCKS_LIGHT[newId] && y > 0 && w.get(x, y - 1, z) === B.DIRT) {
  ...
  private update(i: number): void {
    const w = this.world;
    const layer = w.sx * w.sz;
    const y = Math.floor(i / layer);
    const rem = i - y * layer;
    const z = Math.floor(rem / w.sx);
    const x = rem - z * w.sx;
    const id = w.blocks[i]!;
    switch (id) {
      case B.SAND:
  ...
  private spread(x: number, y: number, z: number, liquid: number): void {
    ...
    for (const [dx, dy, dz] of FLOW_DIRS) {
      ...
      const ny = y + dy;
      const nz = z + dz;
      if (!w.inBounds(nx, ny, nz) || w.get(nx, ny, nz) !== B.AIR) continue;
  ...
  private absorb(x: number, y: number, z: number): void {
    ...
    for (let dy = -r; dy <= r; dy++) {
      for (let dz = -r; dz <= r; dz++) {
        for (let dx = -r; dx <= r; dx++) {
          if (this.world.get(x + dx, y + dy, z + dz) === B.WATER) this.world.setBlock(x + dx, y + dy, z + dz, B.AIR);
        }
      }
    }
  }
  ...
  private covered(x: number, y: number, z: number): boolean {
    return BLOCKS_LIGHT[this.world.get(x, y + 1, z)] === 1 || IS_LIQUID[this.world.get(x, y + 1, z)] === 1;
  }
  ...
  private spreadGrass(x: number, y: number, z: number): void {
    ...
    if (this.world.get(tx, ty, tz) === B.DIRT && this.exposed(tx, ty, tz)) this.world.setBlock(tx, ty, tz, B.GRASS);
  }
    ...
    for (let dy = -1; dy <= 3; dy++) {
      for (let dz = -1; dz <= 1; dz++) {
        for (let dx = -1; dx <= 1; dx++) {
          if (this.world.get(x + dx, y + dy, z + dz) === B.GRASS) return true;
        }
      }
    }
    return false;
  ...
  private randomTicks(): void {
    const w = this.world;
    const samples = Math.max(16, (w.sx * w.sz) >> 6);
    const heights = w.heightMap.heights;
    for (let s = 0; s < samples; s++) {
      const x = this.rng.int(w.sx);
      const z = this.rng.int(w.sz);
      const y = heights[z * w.sx + x]!;
      if (y < 0) continue;
      const top = w.get(x, y, z);
      if (top === B.GRASS) this.spreadGrass(x, y, z);
      if (w.get(x, y + 1, z) === B.SAPLING && this.rng.chance(0.1)) this.growSapling(x, y + 1, z);
    }
  }
  ...
  private touches(x: number, y: number, z: number, id: number): boolean {
    for (const [dx, dy, dz] of NEIGHBOURS) if (this.world.get(x + dx, y + dy, z + dz) === id) return true;
    return false;
  }

  /** An air cell on the map border, level with the ocean outside. */
  private touchesEdgeOcean(x: number, y: number, z: number): boolean {
    const w = this.world;
    if (y < w.edgeFloor || y >= w.seaLevel) return false;
    return x === 0 || z === 0 || x === w.sx - 1 || z === w.sz - 1;
  }
  ...
  private nearSponge(x: number, y: number, z: number): boolean {
    ...
    if (this.sponges.size > 64) {
      for (let dy = -r; dy <= r; dy++)
        for (let dz = -r; dz <= r; dz++)
          for (let dx = -r; dx <= r; dx++) if (w.get(x + dx, y + dy, z + dz) === B.SPONGE) return true;
      return false;
    }
    const layer = w.sx * w.sz;
    for (const i of this.sponges) {
      const sy = Math.floor(i / layer);
      const rem = i - sy * layer;
      const sz = Math.floor(rem / w.sx);
      const sx = rem - sz * w.sx;
